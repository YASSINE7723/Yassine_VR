...
public class VRWC_WheelInteractable : XRBaseInteractable
{
    ...
    private void Start()
    {
        ...
        // Sécurité : vérifie si le Collider existe pour éviter un crash
    ...
    protected override void OnSelectEntered(SelectEnterEventArgs eventArgs)
    {
        ...
        // Désélectionne la roue pour transférer le contrôle au GrabPoint
        interactionManager.CancelInteractableSelection(this);

        SpawnGrabPoint(interactor);

        // J'ai supprimé "BrakeAssist" ici pour corriger l'erreur NullReference
    ...
    void SpawnGrabPoint(XRBaseInteractor interactor)
    {
        ...
        // J'ai retiré "typeof(Rigidbody)" de cette liste car VRWC_GrabPoint l'ajoute déjà automatiquement.
        // Cela corrige l'erreur "Can't add component Rigidbody... already added".
        grabPoint = new GameObject($"{transform.name}'s grabPoint", typeof(VRWC_GrabPoint), typeof(FixedJoint));
